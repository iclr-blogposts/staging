---
layout: distill
title: How Oversmoothing Influences Oversquashing in Graph Neural Networks
description: Oversmoothing and Oversquashing are well-studied areas within graph deep learning. Addressing these issues allows graph neural networks to learn both short-range and long-range information despite the presence of bottleneck edges, ie, edges that are under heavy pressure to propagate information between two or more large communities. Alon and Yahav (2021) extensively study oversquashing and allude to oversmoothing and oversquashing being independent problems that occur in a graph without much influence on each other. In this blog post, we introduce a simple problem setting to demonstrate how early oversmoothing could possibly influence oversquashing by studying their relative rates at each layer in popular graph neural networks. We hypothesize that early oversmoothing results in the collapse of exponentially large rooted subtrees resulting in ease of flow through bottleneck edges, thus slowing down the rate of oversquashing in the network. In doing so, we hope to spur interesting research to further study their effects on each other and build networks that can overcome these issues.
date: 2022-12-01
htmlwidgets: true
